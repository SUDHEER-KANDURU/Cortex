...
function AnimatedNumber({ value, suffix = "", active }: { value: number; suffix?: string; active: boolean }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (p < 1) { rafId.current = requestAnimationFrame(tick) } else { rafId.current = null }
    }
...
function ParsingAnimation({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    if (!active) return
    const t = setInterval(() => {
      // Don't waste CPU cycling when the tab is not visible
  ...
  return (
    <div style={{ background: "#0a0a0a", borderRadius: "16px", overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 8px 40px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.04)" }}>
      ...
      <div style={{ padding: "16px 20px" }}>
        {lines.map((line, i) => (
          <div key={i} style={{
            ...
            {line.text}
            {i === activeLine && <span style={{ display: "inline-block", width: "7px", height: "13px", background: line.color, marginLeft: "2px", verticalAlign: "middle", animation: "caret-blink 0.9s step-end infinite" }} />}
          </div>
        ))}
      </div>
...
const skills = ["AST Parsing","Neo4j Graph DB","FastAPI","Python 3.12","Celery Workers","PostgreSQL","Redis","React Flow","Next.js 14","Architecture Analysis","Knowledge Graphs","LLM-Free"]
const stats = [
...
export function PortfolioAbout() {
  ...
  return (
    <section id="about" className="py-20 md:py-10 md:pb-32 md:pt-32"
      style={{ borderTop: "1px solid var(--cx-card-border)", background: "var(--cx-section-bg)", backdropFilter: "saturate(200%) blur(24px)", WebkitBackdropFilter: "saturate(200%) blur(24px)" }}>
      <div className="max-w-[1280px] mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 mb-16 md:mb-24">
          <div data-reveal="up">
            ...
            <p className="cx-text-muted mt-6 leading-relaxed">Cortex reads your repository at the AST level, constructs a rich knowledge graph in Neo4j, and generates artifacts that explain the system — not just describe it.</p>
            <p className="cx-text-muted mt-4 leading-relaxed">Built at SRMIST Chennai. Runs entirely on your machine. No API keys, no cloud billing, no data leaving your environment.</p>
            <div ref={statsRef} className="grid grid-cols-3 gap-3 mt-10" data-stagger>
              {stats.map(stat => (
                <div key={stat.label} className="cx-stat-card text-center p-4 rounded-2xl transition-all duration-300 hover:-translate-y-1"
        ...
        <div data-reveal="up">
          <h3 className="cx-text-muted text-sm font-semibold uppercase tracking-wider mb-5">Built on production-grade open-source</h3>
          <div className="flex flex-wrap gap-2" data-stagger>
            {skills.map(skill => (
              <span key={skill} className="cx-tag px-4 py-2 text-sm font-medium rounded-full" style={{ backdropFilter: "blur(12px) saturate(160%)", WebkitBackdropFilter: "blur(12px) saturate(160%)", cursor: "default" }}
